...
locals {
  ...
  # replace a '*' with the word 'wildcard' followed by a hash of the domain, meaning if the site is
  # hosted for *.example.com we will create a health check for wildcard-hash.example.com. This will
  # work as intended except when the is a more specific wildcard-hash.example.com record, which we
  # just assume is rare enough that it's not worth worrying about.
...
data "aws_route53_zone" "sans" {
  for_each = var.sans_domain_names_to_zone_names
